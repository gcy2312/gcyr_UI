...
const Toast = (props:ToastProps):JSX.Element =>{
    ...
    const IconAvatar = () =>{
        if(avatar === true){
            return(
                <div className='h-12 w-12 overflow-hidden rounded-full flex-shrink-0'>{Icon}</div>
            )
        }
        else if (!avatar && Icon){
            return (
    ...
    return (
        <div id={id} className='relative inset-0 flex items-end px-4 py-6 pointer-events-none sm:p-4 sm:items-start'>
              ...
              >
                <div className='max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto border border-gray-600/5 overflow-hidden'>
                    ...
                    <div className='ml-4 absolute top-8 right-8 flex-shrink-0'>
                        <button className='bg-white hover:opacity-80' onClick={()=>setOpen(false)}>
